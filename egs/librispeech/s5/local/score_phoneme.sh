...
symtab=$lang_or_graph/phones.txt #words.txt
...
cat $data/text.phonemes | sed 's:<NOISE>::g' | sed 's:<SPOKEN_NOISE>::g' > $dir/scoring/test_filt.txt
...
for wip in $(echo $word_ins_penalty | sed 's/,/ /g'); do
  $cmd LMWT=$min_lmwt:$max_lmwt $dir/scoring/log/score.LMWT.$wip.log \
    ...
    sed 's:_B::g' \| sed 's:_I::g' \| sed 's:_E::g' \| \
