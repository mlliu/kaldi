...
if [ ${stage} -le 6 ] && [ ${stop_stage} -ge 6 ] && ! [[ " ${skip_stages} " =~ [[:space:]]6[[:space:]] ]]; then
  ...
  #for part in train_clean_100_sp0.9 train_clean_100_sp1.1; do
  for part in eval2000; do # train_nodup train_nodup_sp0.9 train_nodup_sp1.1 train_dev eval2000; do
    ...
    wavlmdir="/export/fs05/mliu121/espnet_data/swbd/dump/extracted/wavlm_large/layer21"
...
#    done < ${source}
    ...
    # run the cmvn job in the background
    $train_cmd JOB=1 ${expdir}/make_cmvn/${part}/log/make_cmvn_${part}.JOB.log \
    steps/compute_cmvn_stats.sh ${datadir}/${part} ${expdir}/make_cmvn/${part} ${featdir} &
  done
  wait # wait for the cmvn job to finish
fi
...
#  for test in ${_dsets}; do
 #for test in train_clean_100_sp0.9 train_clean_100_sp1.1; do
 for test in train_nodup train_nodup_sp0.9 train_nodup_sp1.1 train_dev eval2000; do
